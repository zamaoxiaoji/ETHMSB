...
namespace HEDB
{
    void EvalChebyshevBaisc(std::vector<seal::Ciphertext>& T1, std::vector<seal::Ciphertext>& T2, seal::Ciphertext& x,
                            uint64_t degree,
    ...
    void DivisionChebyshevLazy(ChebyshevPoly& f, uint64_t ChebyshevDegree, ChebyshevPoly& quotient,
    ...
    void EvalRecurseLazy(double target_scale, uint64_t m, uint64_t l, ChebyshevPoly& poly,
                         ...
                         seal::SEALContext& context, seal::CKKSEncoder& encoder, seal::Evaluator& evaluator,
                         seal::RelinKeys& relin_keys)
    {
        ...
        if (d < (1ULL << l))
        {
            ...
            for (size_t i = d; i > 0; i--)
            {
                if (std::abs(std::round(poly.coefficients[i] * target_scale)) > 1.)
                {
                    ...
                    seal::multiply_scalar(temp, poly.coefficients[i], cipher_scale / T1[i - 1].scale(), encoder,
                                          evaluator);
        ...
        EvalRecurseLazy(target_scale, m - 1, l, remainder, T1, T2, cipher_remainder, context, encoder, evaluator,
                        relin_keys);
        EvalRecurseLazy(quotient_scale, m - 1, l, quotient, T1, T2, cipher_quotient, context, encoder, evaluator,
                        relin_keys);
        ...
        if (cipher_quotient.size() && cipher_remainder.size())
        {
            ...
            {
                seal::multiply_scalar(cipher_remainder, 1., result.scale() / cipher_remainder.scale(), encoder,
                                      evaluator);
    ...
    void poly_evaluate_bsgs_lazy(seal::Ciphertext& result, double target_scale, seal::Ciphertext& x,
                                 ChebyshevPoly& poly,
                                 seal::SEALContext& context, seal::CKKSEncoder& encoder, seal::Evaluator& evaluator,
                                 seal::RelinKeys& relin_keys)
    {
    ...
    void EvalPower(double target_scale, std::vector<double> coefficients, std::vector<seal::Ciphertext>& power_basis,
                   seal::Ciphertext& result,
                   seal::SEALContext& context, seal::CKKSEncoder& encoder, seal::Evaluator& evaluator,
                   seal::RelinKeys& relin_keys, seal::Decryptor& decryptor)
    {
        if (coefficients.size() == 1)
        {
            if (std::abs(std::round(coefficients[0] * target_scale)) > 1.)
            {
                ...
                seal::multiply_scalar(result, coefficients[0], cipher_scale / power_basis[0].scale(), encoder,
                                      evaluator);
                evaluator.rescale_to_next_inplace(result);
                return;
            }
        ...
        EvalPower(quotient_scale, quotient, power_basis, cipher_quotient, context, encoder, evaluator, relin_keys,
                  decryptor);
        EvalPower(remainder_scale, remainder, power_basis, cipher_remainder, context, encoder, evaluator, relin_keys,
                  decryptor);
    ...
    void poly_evaluate_power(seal::Ciphertext& result, double target_scale, seal::Ciphertext& x,
                             ...
                             seal::RelinKeys& relin_keys, seal::Decryptor& decryptor)
    {
        ...
        {
            seal::multiply_and_relinearize(power_basis[i - 1], power_basis[i - 1], power_basis[i], evaluator,
                                           relin_keys);
