#include "HEDB/comparison/comparison.h"
#include "HEDB/utils/utils.h"
#include "HEDB/conversion/repack.h"

using namespace HEDB;
using namespace std;

/***
     select   
    sum(l_extendedprice*(1-l_discount)) as sum_disc_price, 
    avg(l_extendedprice) as avg_price,
    from
        lineitem
    where
        l_shipdate <= date '1998-12-01' - interval '[DELTA]' day (3)  //DELTA是60~120内的值
    
*/


uint64_t generate_date(uint64_t down, uint64_t up)
{
    uint64_t dyear, dmonth, dday, uyear, umonth, uday;
    dyear = down / 10000;
    dmonth = (down / 100) % 100;
    dday = down % 100;
    uyear = up / 10000;
    umonth = (up / 100) % 100;
    uday = up % 100;
    std::random_device seed_gen;
    std::default_random_engine engine(seed_gen());
    uniform_int_distribution<Lvl1::T> day_message(dday, uday);
    uniform_int_distribution<Lvl1::T> month_message(dmonth, umonth);
    uniform_int_distribution<Lvl1::T> year_message(dyear, uyear);
    return day_message(engine) + 100 * month_message(engine) + 10000 * year_message(engine);

}

void predicate_evaluation(std::vector<TLWELvl1> &pred_cres, std::vector<TLWELvl1> &exist_cres,  std::vector<uint32_t> &pred_res, std::vector<uint32_t> &exist_res, size_t rows, 
    std::vector<uint64_t> ship_data, std::vector<uint32_t> discount_data, TFHESecretKey &sk, double &filter_time)
{
    std::cout<< "Predicate evaluation: " << std::endl;
    using P = Lvl2;
    TFHEEvalKey ek;
    std::cout<< "Generating evaluation key..." << std::endl;
    ek.emplacebkfft<Lvl01>(sk);
    ek.emplacebkfft<Lvl02>(sk);
    ek.emplaceiksk<Lvl20>(sk);
    ek.emplaceiksk<Lvl10>(sk);
    ek.emplaceiksk<Lvl21>(sk);
    
    uint32_t ship_bits = 16, discount_bits = 16, ship_scale_bits, discount_scale_bits;

    
    ship_scale_bits = std::numeric_limits<Lvl2::T>::digits - ship_bits - 1;
    discount_scale_bits = std::numeric_limits<Lvl2::T>::digits - discount_bits - 1;


    //Encrypt database
    std::cout<< "Encrypting Database..." << std::endl;
    
    std::vector<TLWELvl2> ship_ciphers(rows);
    std::vector<TLWELvl2> discount_ciphers(rows);
    for (size_t i = 0; i < rows; i++)
    {
        ship_ciphers[i] = TFHEpp::tlweSymInt32Encrypt<Lvl2>(ship_data[i], Lvl2::α, pow(2., ship_scale_bits), sk.key.get<Lvl2>());
        discount_ciphers[i] = TFHEpp::tlweSymInt32Encrypt<Lvl2>(discount_data[i], Lvl2::α, pow(2., discount_scale_bits), sk.key.get<Lvl2>());
    }
    
    //Encrypt Predicate values
    std::cout<< "Encrypting Predicate Values..." << std::endl;
    Lvl1::T pred3, pred4, pred5;
    Lvl2::T pred1 = 20101;
    //Lvl2::T pred2 = 21231;
    pred3 =  8;
    // pred4 = pred3 + 2;
    // pred5 = 32;

    std::vector<Lvl1::T> pred_res1(rows, 0), pred_res2(rows, 0), pred_res3(rows, 0), pred_res4(rows, 0), pred_res5(rows, 0);
    std::vector<std::vector<uint32_t>> p_exist_matrix(rows);
    std::unordered_set<uint32_t> unique_set;
    uint32_t t = 0;

    std::random_device seed_gen;
    std::default_random_engine engine(seed_gen());
    uniform_int_distribution<Lvl1::T> message(0,  1);
    for (size_t i = 0; i < rows; i++)
    {

        //pred_res[i] = (discount_data[i] > pred3) ? 1 : 0;

        pred_res[i] = message(engine);
        pred_cres[i] = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred_res[i], Lvl1::α, pow(2., 29), sk.key.get<Lvl1>());
    

        // if (unique_set.find(discount_data[i]) != unique_set.end()) {
        //     exist_res[i] = 1;  // 已存在
        // } else {
        //     exist_res[i] = 0;  // 不存在
        //     unique_set.insert(discount_data[i]); // 插入集合
        // }
        //exist_res[i] = exist_res[i] * pred_res[i];

        if( i > 0){
            p_exist_matrix[i].resize(i);
            t = 0;
            for(size_t j = 0 ; j< i ;j++){
                p_exist_matrix[i][j] = (discount_data[i] == discount_data[j]) ? 1 : 0;
                p_exist_matrix[i][j] = p_exist_matrix[i][j] * pred_res[i] *pred_res[j];
                t += p_exist_matrix[i][j];
            }
            exist_res[i] = (t > 0 ) ? 1 : 0;
        }

        



    }
    
    
    TLWELvl2 pred_cipher1 , pred_cipher2;
    TLWELvl1 pred_cipher3, pred_cipher4, pred_cipher5;
    TLWELvl1 number_one_cipher_at28, number_one_cipher_at29, number_zero_cipher_at28, number_zero_cipher_at29, number_two_cipher_at29;

    //pred_cipher1 = TFHEpp::tlweSymInt32Encrypt<Lvl2>(pred1, Lvl2::α, pow(2., ship_scale_bits), sk.key.get<Lvl2>());
    // pred_cipher2 = TFHEpp::tlweSymInt32Encrypt<Lvl2>(pred2, Lvl2::α, pow(2., ship_scale_bits), sk.key.get<Lvl2>());
    pred_cipher3 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred3, Lvl1::α, pow(2., discount_scale_bits), sk.key.get<Lvl1>());
    // pred_cipher4 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred4, Lvl1::α, pow(2., discount_scale_bits), sk.key.get<Lvl1>());
    // pred_cipher5 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred5, Lvl1::α, pow(2., quantity_scale_bits), sk.key.get<Lvl1>());
    number_one_cipher_at28 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(1, Lvl1::α, pow(2., 28), sk.key.get<Lvl1>());
    number_one_cipher_at29 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(1, Lvl1::α, pow(2., 29), sk.key.get<Lvl1>());
    number_zero_cipher_at28 = TFHEpp::tlweSymInt32Encrypt<Lvl1>( 0 , Lvl1::α, pow(2., 28), sk.key.get<Lvl1>());
    number_zero_cipher_at29 = TFHEpp::tlweSymInt32Encrypt<Lvl1>( 0 , Lvl1::α, pow(2., 29), sk.key.get<Lvl1>());

    number_two_cipher_at29 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(2, Lvl1::α, pow(2., 29), sk.key.get<Lvl1>());

    // Predicate Evaluation
    std::cout<< "Start Predicate Evaluation..." << std::endl;
    std::vector<std::vector<TLWELvl1>> exist_matrix(rows);
    std::vector<TLWELvl1> pred_cres1(rows), pred_cres3(rows);
    std::chrono::system_clock::time_point start, end;

    exist_cres[0] = number_zero_cipher_at29;
    start = std::chrono::system_clock::now();

    
    std::vector<TLWELvl1> arr_c(3);
    
    for (size_t i = 0; i < rows; i++)
    {   
        //my_greater_than<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres[i], discount_bits, ek, ARITHMETIC, 29);
        
        arr_c[0] = pred_cres[i];
        if( i > 0){
            exist_matrix[i].resize(i);
            for(size_t j = 0 ; j< i ;j++){
                // my_equal<Lvl1>(discount_ciphers[i] ,discount_ciphers[j], exist_matrix[i][j], number_one_cipher_at29 ,discount_bits, ek, ARITHMETIC, 29);
                // my_HomAND(exist_matrix[i][j], exist_matrix[i][j], pred_cres[i], number_one_cipher_at29, ek, ARITHMETIC, 29);
                // my_HomAND(exist_matrix[i][j], exist_matrix[i][j], pred_cres[j], number_one_cipher_at29, ek, ARITHMETIC, 29);
                
                
                my_equal<Lvl2>(discount_ciphers[i] ,discount_ciphers[j], exist_matrix[i][j], number_one_cipher_at29 ,discount_bits, ek, ARITHMETIC, 29);
                arr_c[1] = pred_cres[j];
                arr_c[2] = exist_matrix[i][j];
                N_HomAND(arr_c, exist_matrix[i][j], number_two_cipher_at29, number_two_cipher_at29, number_two_cipher_at29, ek, 29);
                
            }
            
            N_HomOR(exist_matrix[i] ,exist_cres[i] , number_zero_cipher_at29,  ek ,29);
        }

        //my_HomAND(exist_cres[i], exist_cres[i], pred_cres[i], number_one_cipher_at29, ek, ARITHMETIC, 29);
        
    }
    end = std::chrono::system_clock::now();
    std::vector<uint32_t> pred_cres_de(rows);
    std::vector<uint32_t> pred_exist_de(rows);
    // for (size_t i = 0; i < rows; i++)
    // {
    //     pred_cres_de[i] = TFHEpp::tlweSymInt32Decrypt<Lvl1>(pred_cres[i], pow(2., 31), sk.key.get<Lvl1>());
    //     // pred_cres1_de[i] =  TFHEpp::tlweSymDecrypt<Lvl1>(pred_cres1[i], sk.key.lvl1);
    //     // pred_cres2_de[i] =  TFHEpp::tlweSymDecrypt<Lvl1>(pred_cres2[i], sk.key.lvl1);
    //     // pred_cres3_de[i] =  TFHEpp::tlweSymDecrypt<Lvl1>(pred_cres3[i], sk.key.lvl1);
    //     // pred_cres4_de[i] =  TFHEpp::tlweSymDecrypt<Lvl1>(pred_cres4[i], sk.key.lvl1);
    //     // pred_cres5_de[i] =  TFHEpp::tlweSymDecrypt<Lvl1>(pred_cres5[i], sk.key.lvl1);
    // }
    
    size_t pred_error_time = 0;
    size_t exist_error_time = 0;

    // uint32_t rlwe_scale_bits = 29;
    // for (size_t i = 0; i < rows; i++)
    // {
    //     TFHEpp::ari_rescale(pred_cres[i], pred_cres[i], rlwe_scale_bits, ek);
    // }
    for (size_t i = 0; i < rows; i++)
    {
        pred_cres_de[i] = TFHEpp::tlweSymInt32Decrypt<Lvl1>(pred_cres[i], pow(2., 29), sk.key.get<Lvl1>());
        pred_exist_de[i] = TFHEpp::tlweSymInt32Decrypt<Lvl1>(exist_cres[i], pow(2., 29), sk.key.get<Lvl1>());
    }

    uint32_t print_val ;
    for (size_t i = 0; i < rows; i++) {
        pred_error_time += (pred_cres_de[i] == pred_res[i])? 0 : 1;
        //exist_error_time += (pred_exist_de[i] == exist_res[i])? 0 : 1;
        if(pred_exist_de[i] != exist_res[i]){
            exist_error_time ++;
            cout << "   discount_data: " ;
            for(int j = 0 ; j < rows ; j++){
                cout << discount_data[j] << "  " ;
            }
            cout << " " << std::endl;

            cout << "     exist_res[" << i <<"]: "<< exist_res[i] << std::endl;
            cout << "p_exist_matrix[" << i <<"]: " ;
            for(int j = 0 ;j < p_exist_matrix[i].size();j++){
                cout << p_exist_matrix[i][j] << "  " ;
            }
            cout << " " << std::endl;

            cout << " pred_exist_de[" << i <<"]: "<< pred_exist_de[i] << std::endl;
            cout << "  exist_matrix[" << i <<"]: " ;
            for(int j = 0 ;j < exist_matrix[i].size();j++){
                print_val =TFHEpp::tlweSymInt32Decrypt<Lvl1>(exist_matrix[i][j], pow(2., 29), sk.key.get<Lvl1>());
                cout << print_val << "  " ;
            }
            cout << " " << std::endl;
        }
    }
    cout << "Predicate Evaluaton Time (s): " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() / 1000 << "s"<< std::endl;
    cout << "Predicate Error: " << pred_error_time << std::endl;
    cout << "exist_error_time: " << exist_error_time << std::endl;
    filter_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

    cout << "pred_res: "   << endl;
    for (size_t i = 0; i < rows; i++)
    {
        cout << pred_res[i] << " ";
    }
    cout << " "   << endl;

    cout << "discount_data: "   << endl;
    for (size_t i = 0; i < rows; i++)
    {
        cout << discount_data[i] << " ";
    }
    cout << " "   << endl;


}

void aggregation(std::vector<TLWELvl1> &pred_cres, std::vector<TLWELvl1> &exist_cres, std::vector<uint32_t> &pred_res, std::vector<uint32_t> &exist_res, size_t tfhe_n,
            std::vector<double> &extendedprice_data, std::vector<double> &discount_data_double,
             size_t rows, TFHESecretKey &sk, double &aggregation_time)
{
    std::cout << "Aggregation :" << std::endl;
    uint64_t scale_bits = 29;
    uint64_t modq_bits = 32;
    uint64_t modulus_bits = 45;
    uint64_t repack_scale_bits = modulus_bits + scale_bits - modq_bits;
    uint64_t slots_count = pred_cres.size();
    std::cout << "Generating Parameters..." << std::endl;
    seal::EncryptionParameters parms(seal::scheme_type::ckks);
    size_t poly_modulus_degree = 65536;
    parms.set_poly_modulus_degree(poly_modulus_degree);
    parms.set_coeff_modulus(seal::CoeffModulus::Create(poly_modulus_degree, {59, 42, 42, 42, 42, 42, 42, 42, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 59}));
    double scale = std::pow(2.0, scale_bits);

    //context instance
    seal::SEALContext context(parms, true, seal::sec_level_type::none);

    //key generation
    seal::KeyGenerator keygen(context);
    seal::SecretKey seal_secret_key = keygen.secret_key();
    seal::RelinKeys relin_keys;
    keygen.create_relin_keys(relin_keys);
    seal::GaloisKeys galois_keys;
    keygen.create_galois_keys(galois_keys);
    

    //utils
    seal::Encryptor encryptor(context, seal_secret_key);
    seal::Evaluator evaluator(context);
    seal::Decryptor decryptor(context, seal_secret_key);

    //encoder
    seal::CKKSEncoder ckks_encoder(context);

    

    //generate evaluation key
    std::cout << "Generating Conversion Key..." << std::endl;
    LTPreKey pre_key;
    LWEsToRLWEKeyGen(pre_key, std::pow(2., modulus_bits), seal_secret_key, sk, tfhe_n, ckks_encoder, encryptor, context);


    // conversion
    std::cout << "Starting Conversion..." << std::endl;
    seal::Ciphertext result;
    seal::Ciphertext result1; ///////////////////////////////////////
     
    std::chrono::system_clock::time_point start, end;
    start = std::chrono::system_clock::now();
    LWEsToRLWE(result, pred_cres, pre_key, scale, std::pow(2., modq_bits), std::pow(2., modulus_bits - modq_bits), ckks_encoder, galois_keys, relin_keys, evaluator, context);
    HomRound(result, result.scale(), ckks_encoder, relin_keys, evaluator, decryptor, context);
    
    //result1 = result; ///////////////////////////////////////////////
    LWEsToRLWE(result1, exist_cres, pre_key, scale, std::pow(2., modq_bits), std::pow(2., modulus_bits - modq_bits), ckks_encoder, galois_keys, relin_keys, evaluator, context);
    HomRound(result1, result1.scale(), ckks_encoder, relin_keys, evaluator, decryptor, context);
    
    end = std::chrono::system_clock::now();
    aggregation_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    seal::Plaintext plain;
    std::vector<double> computed(slots_count);
    decryptor.decrypt(result, plain);
    seal::pack_decode(computed, plain, ckks_encoder);

    double err = 0.;
    
    for (size_t i = 0; i < slots_count; ++i)
    {
        err += std::abs(computed[i] - pred_res[i]);
    }

    printf("Repack average error = %f ~ 2^%.1f\n", err / slots_count, std::log2(err / slots_count));


    // Filter result * data
    std::vector<double> price_discount(extendedprice_data.size());
    
    ////////////////////////////////////////////////////////////////
    std::vector<double> count_item(extendedprice_data.size());
    
    
    seal::Ciphertext price_discount_cipher;
    seal::Ciphertext count_item_cipher;/////////////////////////////
    
    for (size_t i = 0; i < rows; i++)
    {
        price_discount[i] = extendedprice_data[i] * discount_data_double[i];
        count_item[i] = 1.;//////////////////////////////
    }
    double qd = parms.coeff_modulus()[result.coeff_modulus_size() - 1].value();
    seal::pack_encode(price_discount, qd, plain, ckks_encoder);
    encryptor.encrypt_symmetric(plain, price_discount_cipher);
    
    ///////////////////////////////////////////////////////////////
    seal::pack_encode(count_item, qd, plain, ckks_encoder);
    encryptor.encrypt_symmetric(plain, count_item_cipher);
    
    
    std::cout << "Aggregating price and discount .." << std::endl;
    start = std::chrono::system_clock::now();
    seal::multiply_and_relinearize(result, count_item_cipher, result, evaluator, relin_keys);
    evaluator.rescale_to_next_inplace(result);
    
    //////////////////////////////////////////////
    seal::multiply_and_relinearize(result1, count_item_cipher, result1, evaluator, relin_keys);
    evaluator.rescale_to_next_inplace(result1);
    
    int logrow = log2(rows);
    
    seal::Ciphertext temp;
    for (size_t i = 0; i < logrow; i++)
    {
        temp = result;
        size_t step = 1 << (logrow - i - 1);
        evaluator.rotate_vector_inplace(temp, step, galois_keys);
        evaluator.add_inplace(result, temp);
    }
    
    //////////////////////////////////////////////
    seal::Ciphertext temp1;
    for (size_t i = 0; i < logrow; i++)
    {
        temp1 = result1;
        size_t step = 1 << (logrow - i - 1);
        evaluator.rotate_vector_inplace(temp1, step, galois_keys);
        evaluator.add_inplace(result1, temp1);
    }
    
    
    end = std::chrono::system_clock::now();
    aggregation_time += std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    std::vector<double> agg_result(slots_count);
    decryptor.decrypt(result, plain);
    seal::pack_decode(agg_result, plain, ckks_encoder);
    
    /////////////////////////////////////////////
    std::vector<double> agg_result1(slots_count);
    decryptor.decrypt(result1, plain);
    seal::pack_decode(agg_result1, plain, ckks_encoder);
    
    
    double plain_result = 0;
    for (size_t i = 0; i < rows; i++)
    {
        plain_result += pred_res[i];
    }
    
    /////////////////////////////////////////////
    double plain_count = 0;
    for (size_t i = 0; i < rows; i++)
    {
        plain_count += exist_res[i];
    }


    cout << "total_counbt: " << plain_result  << endl;
    cout << "repeat_counbt: " << plain_count  << endl;
    cout << "Plain_result: " << plain_result - plain_count << endl;
    
    cout << "Encrypted total_counbt: " << std::round(agg_result[0])  << endl;
    cout << "Encrypted repeat_counbt: " << std::round(agg_result1[0])  << endl;
    cout << "Encrypted query result: " << std::round(agg_result[0]) - std::round(agg_result1[0]) <<endl;
    
    ///////////////////////////////////////////////////////
    // cout << "Plain_count: " << plain_count << endl;
    // cout << "Encrypted count result: " << std::round(agg_result1[0]) <<endl;

}

void query_evaluation(size_t rows)
{
    TFHESecretKey sk;
    std::random_device seed_gen;
    std::default_random_engine engine(seed_gen());
     // Generate database
    std::vector<uint32_t> discount_data(rows);
    std::vector<uint64_t> ship_data(rows);
    uint32_t ship_bits = 16, discount_bits = 8, ship_scale_bits, discount_scale_bits;
    
    uniform_int_distribution<Lvl1::T> discount_message(0, (1 << 4) - 1);
    
    ship_scale_bits = std::numeric_limits<Lvl2::T>::digits - ship_bits - 1;
    discount_scale_bits = std::numeric_limits<Lvl1::T>::digits - discount_bits - 1;
    for (size_t i = 0; i < rows; i++)
    {
        
        discount_data[i] = discount_message(engine);
        ship_data[i] = generate_date(10101, 21230);
    }

    std::vector<double> extendedprice_data(rows), discount_data_double(rows);
    std::uniform_int_distribution<uint64_t> extendedprice_message(1, 10);
    for (size_t i = 0; i < rows; i++)
    {
        extendedprice_data[i] = (double)extendedprice_message(engine);
        discount_data_double[i] = (double)discount_data[i];
    }

    
    discount_data[0] = 9;
    ship_data[0] = 21215;
    double filter_time, aggregation_time;
    std::vector<TLWELvl1> pred_cres(rows);
    std::vector<TLWELvl1> exist_cres(rows);
    std::vector<uint32_t> pred_res(rows, 0);
    std::vector<uint32_t> exist_res(rows, 0);
    predicate_evaluation(pred_cres, exist_cres, pred_res, exist_res, rows, ship_data, discount_data, sk, filter_time);
    aggregation(pred_cres, exist_cres, pred_res, exist_res, Lvl1::n, extendedprice_data, discount_data_double, rows, sk, aggregation_time);
    cout << "End-to-End Time: " << (filter_time + aggregation_time) / 1000 << " s" << endl;

}

int main()
{
    query_evaluation(64);
    //query_evaluation(256);
}