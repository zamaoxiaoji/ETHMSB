    ...
    std::vector<uint64_t> ship_data, TFHESecretKey &sk, double &filter_time)
{
    ...
    //////////////////////////////////////////////////////
    pred_nums_cipher = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred_nums, Lvl1::α, pow(2., quantity_scale_bits), sk.key.get<Lvl1>());
    ////////////////////////////////////////////////////////
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        //less_than<Lvl2>(ship_ciphers[i], pred_cipher2, pred_cres2[i], ship_bits, ek, LOGIC);
        //HomAND(pred_cres[i], pred_cres1[i], pred_cres2[i], ek, LOGIC);
        my_greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres3[i], discount_bits, ek, LOGIC,1);
        //greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres3[i], discount_bits, ek, LOGIC);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres3[i], ek, LOGIC);
        my_less_than_equal<Lvl1>(discount_ciphers[i], pred_cipher4, pred_cres4[i], discount_bits, ek, LOGIC, 1);
        //less_than_equal<Lvl1>(discount_ciphers[i], pred_cipher4, pred_cres4[i], discount_bits, ek, LOGIC);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres4[i], ek, LOGIC);
        my_less_than<Lvl1>(quanlity_ciphers[i], pred_cipher5, pred_cres5[i], quantity_bits, ek, LOGIC, 1);
        //less_than<Lvl1>(quanlity_ciphers[i], pred_cipher5, pred_cres5[i], quantity_bits, ek, LOGIC);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres5[i], ek, ARITHMETIC);

        for (size_t j = 0; j <= Lvl1 :: n; j++)
        {
            //pred_cres[i][j] = pred_cres1[i][j] + pred_cres2[i][j] + pred_cres3[i][j] + pred_cres4[i][j] + pred_cres5[i][j];
            pred_cres[i][j] =  pred_cres3[i][j] + pred_cres4[i][j] + pred_cres5[i][j];
        }

        //greater_than<Lvl1>(pred_cres[i], pred_nums_cipher, pred_cres[i], 4, ek, LOGIC);
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        pred_cres_de[i] = TFHEpp::tlweSymDecrypt_print<Lvl1>(pred_cres[i], sk.key.lvl1);
    ...
    for (size_t i = 0; i < rows; i++)
    {
        cout << i <<"------ before --- ari_rescale ------" << std::endl;
        TFHEpp::tlweSymDecrypt_print<Lvl1>(pred_cres[i], sk.key.lvl1);
        
        TFHEpp::ari_rescale(pred_cres[i], pred_cres[i], rlwe_scale_bits, ek);

        cout << i << "------ after --- ari_rescale -------" << std::endl;
        TFHEpp::tlweSymDecrypt_print<Lvl1>(pred_cres[i], sk.key.lvl1);
    }
    */
    ...
    //     //pred_cres_de[i] = TFHEpp::tlweSymInt32Decrypt<Lvl1>(pred_cres[i], pow(2., 28), sk.key.get<Lvl1>());
    // }
    ...
    cout << "filter_time_original: " << filter_time_original << std::endl;
    cout << "filter_time: " << filter_time << std::endl;
}
