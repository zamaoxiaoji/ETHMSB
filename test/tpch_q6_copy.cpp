    ...
    std::vector<uint64_t> ship_data, std::vector<uint32_t> discount_data, TFHESecretKey &sk, double &filter_time)
{
    ...
    uint32_t ship_bits = 16, discount_bits = 4, ship_scale_bits, discount_scale_bits;
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        pred_res[i] = pred_res1[i] * pred_res3[i];
    }
    ...
    for (size_t i = 0; i < rows; i++)
    {
        less_than_equal<Lvl2>(ship_ciphers[i], pred_cipher1, pred_cres1[i], ship_bits, ek, LOGIC);
        ...
        HomAND(pred_cres[i], pred_cres1[i], pred_cres3[i], ek, ARITHMETIC);
    }
...
void query_evaluation(size_t rows)
{
    ...
    predicate_evaluation(pred_cres, pred_res, rows, ship_data, discount_data, sk, filter_time);
