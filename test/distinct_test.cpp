...
void predicate_evaluation(std::vector<TLWELvl1> &pred_cres, std::vector<TLWELvl1> &exist_cres, std::vector<uint32_t> &pred_res, std::vector<uint32_t> &exist_res, 
                        size_t rows, std::vector<uint32_t> quanlity_data, std::vector<uint32_t> discount_data, 
                        std::vector<uint64_t> ship_data, TFHESecretKey &sk, double &filter_time)
{
    ...
    //std::vector<Lvl1::T> exist(rows, 0);
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        pred_res[i] = (quanlity_data[i] > pred5) ? 1 : 0;
        // if (unique_set.find(quanlity_data[i]) != unique_set.end()) {
        //     exist_res[i] = 1;  // 已存在
        // } else {
        //     exist_res[i] = 0;  // 不存在
        //     unique_set.insert(quanlity_data[i]); // 插入集合
        // }
        if( i > 0){
            p_exist_matrix[i].resize(i);
            t = 0;
            for(size_t j = 0 ; j< i ;j++){
                p_exist_matrix[i][j] = (quanlity_data[i] == quanlity_data[j]) ? 1 : 0;
    ...
    TLWELvl1 number_one_cipher_at28, number_one_cipher_at29, number_zero_cipher_at28, number_zero_cipher_at29;
    ...
    for (size_t i = 0; i < rows; i++)
    {   
        my_greater_than<Lvl1>(quanlity_ciphers[i], pred_cipher5, pred_cres[i], quantity_bits, ek, ARITHMETIC, 29);
        //std::cout<< "i: " << i << std::endl;
        if( i > 0){
            exist_matrix[i].resize(i);
            for(size_t j = 0 ; j< i ;j++){
                //std::cout<< j << "  ";
                my_equal<Lvl1>(quanlity_ciphers[i] ,quanlity_ciphers[j], exist_matrix[i][j], number_one_cipher_at28 ,quantity_bits, ek, ARITHMETIC, 28);
            }
            //std::cout<< " " << std::endl;
            //std::cout<< "N_HomOR_ " << i << std::endl;
            N_HomOR(exist_matrix[i] ,exist_cres[i] , number_zero_cipher_at28,  ek ,29);
            //std::cout<< "N_HomOR_ " << i <<" end" << std::endl;
        }
    ...
    std::vector<uint32_t> pred_cres_de(rows), pred_exist_de(rows);
...
void query_evaluation(size_t rows)
{
    ...
    uint32_t quantity_bits = 6, ship_bits = 16, discount_bits = 4, quantity_scale_bits, ship_scale_bits, discount_scale_bits;
    uniform_int_distribution<Lvl1::T> quantity_message(0, (1 << quantity_bits) - 40);
    ...
    std::vector<TLWELvl1> pred_cres(rows), exist_cres(rows);
    std::vector<uint32_t> pred_res(rows, 0) , exist_res(rows, 0);
    
    predicate_evaluation(pred_cres, exist_cres, pred_res, exist_res, rows, quanlity_data, discount_data, ship_data, sk, filter_time);
