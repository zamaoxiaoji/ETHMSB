                          ...
                          std::vector<uint64_t> ship_data, TFHESecretKey& sk, double& filter_time)
{
    ...
    uint32_t quantity_bits = 6, ship_bits = 16, discount_bits = 4, quantity_scale_bits, ship_scale_bits,
             discount_scale_bits;
    double original_filter_time = 0;
...
#pragma omp parallel for schedule(static) if(use_parallel_enc) num_threads(omp_threads)
    ...
    for (size_t i = 0; i < rows; i++)
    {
        quanlity_ciphers[i] = TFHEpp::tlweSymInt32Encrypt<Lvl1>(quanlity_data[i], Lvl1::α, pow(2., quantity_scale_bits),
                                                                sk.key.get<Lvl1>());
        discount_ciphers[i] = TFHEpp::tlweSymInt32Encrypt<Lvl1>(discount_data[i], Lvl1::α, pow(2., discount_scale_bits),
                                                                sk.key.get<Lvl1>());
        ship_ciphers[i] = TFHEpp::tlweSymInt32Encrypt<Lvl2>(ship_data[i], Lvl2::α, pow(2., ship_scale_bits),
                                                            sk.key.get<Lvl2>());
    }

    //Encrypt Predicate values
    std::cout << "Encrypting Predicate Values..." << std::endl;
    Lvl1::T pred3, pred4, pred5, pred_nums;
    ...
    std::vector<Lvl1::T> pred_res1(rows, 0), pred_res2(rows, 0), pred_res3(rows, 0), pred_res4(rows, 0),
                         pred_res5(rows, 0);
    for (size_t i = 0; i < rows; i++)
    {
    ...
    pred_nums_cipher = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred_nums, Lvl1::α, pow(2., 28), sk.key.get<Lvl1>());
...
#pragma omp parallel for schedule(static) if(use_parallel_filter_logic) num_threads(omp_threads)
    ...
    original_filter_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
...
#pragma omp parallel for schedule(static) if(use_parallel_filter) num_threads(omp_threads)
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        //HomAND(pred_cres[i], pred_cres1[i], pred_cres2[i], ek, ARITHMETIC);
        my_greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres3[i], discount_bits, ek, ARITHMETIC,
                                    28);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres3[i], ek, ARITHMETIC);
        my_less_than_equal<Lvl1>(discount_ciphers[i], pred_cipher4, pred_cres4[i], discount_bits, ek, ARITHMETIC, 28);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres4[i], ek, ARITHMETIC);
        my_less_than<Lvl1>(quanlity_ciphers[i], pred_cipher5, pred_cres5[i], quantity_bits, ek, ARITHMETIC, 28);
        //HomAND(pred_cres[i], pred_cres[i], pred_cres5[i], ek, ARITHMETIC);
        for (size_t j = 0; j <= Lvl1::n; j++)
        {
            pred_cres_add[i][j] = pred_cres1[i][j] + pred_cres2[i][j] + pred_cres3[i][j] + pred_cres4[i][j] + pred_cres5
                [i][j];
        }

        my_greater_than<Lvl1>(pred_cres_add[i], pred_nums_cipher, pred_cres[i], 3, ek, ARITHMETIC, 29);
    }

    end = std::chrono::system_clock::now();

    filter_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    cout << "Predicate Evaluaton Time (s): " << filter_time / 1000 << std::endl;
    ...
    for (size_t i = 0; i < rows; i++)
    {
        if (pred_cres_de[i] != pred_res[i])
        {
            ...
            std::cout << "pred_res[" << i << "]:" << pred_res[i] << " pred_cres_de[" << i << "]:" << pred_cres_de[i] <<
                std::endl;
            std::cout << "pred_res1[" << i << "]:" << pred_res1[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres1[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_res2[" << i << "]:" << pred_res2[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres2[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_res3[" << i << "]:" << pred_res3[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres3[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_res4[" << i << "]:" << pred_res4[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres4[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_res5[" << i << "]:" << pred_res5[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres5[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_cres_add[" << i << "]:" << pred_res[i] << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres_add[i], pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "pred_nums_cipher:" << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_nums_cipher, pow(2., 28), sk.key.get<Lvl1>());


            TLWELvl1 c_sub, sign_tlwe;
            for (size_t j = 0; j <= Lvl1::k * Lvl1::n; j++)
            {
                c_sub[j] = pred_nums_cipher[j] - pred_cres_add[i][j];
            }
            std::cout << "---------sub---------:" << std::endl;
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(c_sub, pow(2., 28), sk.key.get<Lvl1>());

            std::cout << "--------boot---------:" << std::endl;
            my_MSBGateBootstrapping(sign_tlwe, c_sub, ek, ARITHMETIC, 2);
            TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(sign_tlwe, pow(2., 29), sk.key.get<Lvl1>());

            std::cout << "pred_cres_de[" << i << "]:" << pred_cres_de[i] << std::endl;
...
#pragma omp parallel for schedule(static) reduction(+:plain_result) if(use_parallel_plain) num_threads(omp_threads)
    ...
    cout << "rotate_sum_time: " << (aggregation_time - LWEsToRLWE_time) << "ms" << endl;
}

void query_evaluation(size_t rows)
{
    cout << "tpch_q6_mytest_v5 " << endl;
    cout << "rows: " << rows << endl;
    TFHESecretKey sk;
    std::random_device seed_gen;
    std::default_random_engine engine(seed_gen());
