...
//#include "HEDB/comparison/operators.h"
...
void predicate_evaluation(std::vector<TLWELvl1> &pred_cres_total, std::vector<uint32_t> &pred_res_total, 
    ...
    std::vector<std::vector<uint32_t>> p_type_data, std::vector<uint64_t> ship_data, TFHESecretKey &sk, double &filter_time)
{
    ...
    uint32_t  ship_bits = 16, p_type_bits = 8, ship_scale_bits, p_type_scale_bits;
    ...
    std::vector<uint32_t> pred3 = {80,82,79,77,79,60,60}; //PROMO 的 ascii码
    

    std::vector<Lvl1::T> pred_res1(rows, 0), pred_res2(rows, 0), pred_res3(rows, 1);
    for (size_t i = 0; i < rows; i++)
    {
        ...
        pred_res_part[i] = pred_res1[i] * pred_res2[i] * pred_res3[i];
        pred_res_total[i] = pred_res1[i] * pred_res2[i];
    }
    
    TLWELvl2 pred_cipher1, pred_cipher2;
    std::vector<TLWELvl1> pred_cipher3(7);
    Lvl1::T  pred_number_total = 1;
    Lvl1::T pred_number_part = 6;
    TLWELvl1 number_one_cipher_at29, pred_number_cipher_part, number_one_cipher_at28;

    number_one_cipher_at29 = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred_number_total, Lvl1::α, pow(2., 29), sk.key.get<Lvl1>());
    pred_number_cipher_part = TFHEpp::tlweSymInt32Encrypt<Lvl1>(pred_number_part, Lvl1::α, pow(2., 28), sk.key.get<Lvl1>());
    ...
    std::vector<TLWELvl1> pred_cres1(rows), pred_cres2(rows), pred_cres3(rows), pred_cres_add_total(rows), pred_cres_add_part(rows), pred_cres_str(rows);

    std::vector<TLWELvl1> pred_cres_char(7);
    std::vector<uint32_t> pred_cres_total_de(rows), pred_cres_part_de(rows);

    std::chrono::system_clock::time_point start, end, start2, end2;
    start = std::chrono::system_clock::now();
    double like_time = 0.0;
    
    for (size_t i = 0; i < rows; i++)
    {
        my_greater_than_equal<Lvl2>(ship_ciphers[i], pred_cipher1, pred_cres1[i], ship_bits, ek, ARITHMETIC, 28);
        my_less_than<Lvl2>(ship_ciphers[i], pred_cipher2, pred_cres2[i], ship_bits, ek, ARITHMETIC, 28);
        // HomAND(pred_cres_total_logic[i], pred_cres1[i], pred_cres2[i], ek, LOGIC);
        // HomAND(pred_cres_total[i], pred_cres1[i], pred_cres2[i], ek, ARITHMETIC);

        for (size_t j = 0; j <= Lvl1 :: n; j++){
            pred_cres_add_total[i][j] = pred_cres1[i][j] +  pred_cres2[i][j] ;
        }

        my_greater_than<Lvl1>(pred_cres_add_total[i], number_one_cipher_at28, pred_cres_total[i], 3, ek, ARITHMETIC, 29);
        

        //my_equal<Lvl1>(p_type_ciphers[i][0] ,pred_cipher3[0] , pred_cres3[i], p_type_bits, ek, ARITHMETIC, 29);
        
        start2 = std::chrono::system_clock::now();
        for(int j = 0; j < 7; j++){
            my_equal<Lvl1>(p_type_ciphers[i][j] ,pred_cipher3[j] , pred_cres_char[j], number_one_cipher_at28, p_type_bits, ek, ARITHMETIC, 28);
        }

        for (size_t j = 0; j <= Lvl1 :: n; j++){
            pred_cres_add_part[i][j] = pred_cres_char[0][j];
            for(size_t k = 1; k <7; k++){
                pred_cres_add_part[i][j] = pred_cres_add_part[i][j] +  pred_cres_char[k][j] ;
            }
            
        }

        my_greater_than<Lvl1>(pred_cres_add_part[i], pred_number_cipher_part, pred_cres_str[i], 3, ek, ARITHMETIC, 29);
        
        end2 = std::chrono::system_clock::now();
        like_time += std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start2).count();


        my_HomAND(pred_cres_part[i], pred_cres_str[i], pred_cres_total[i], number_one_cipher_at29, ek, ARITHMETIC, 29);
        ...
        // if(pred_cres_part_de[i] != pred_res_part[i]){
        //     std::cout << "pred3: " << std::endl;
        //     for(size_t j = 0; j < 5; j++){
        //         std::cout << pred3[j]<< " " ;
        //     }
        //     std::cout << std::endl;

        //     std::cout << "p_type_data: " << std::endl;
        //     for(size_t j = 0; j < 5; j++){
        //         std::cout << p_type_data[i][j] << " " ;
        //     }
        //     std::cout <<  std::endl;

        //     std::cout << "equal: " << std::endl;
        //     std::vector<uint32_t> de_equal(5);
        //     for(size_t j = 0; j < 5; j++){
        //         de_equal[j] = TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres_char[j], pow(2., 28), sk.key.get<Lvl1>());
        //         std::cout << de_equal[j] << " " ;
        //     }
        //     std::cout << std::endl;

        //     uint32_t char_add = TFHEpp::tlweSymInt32Decrypt<Lvl1>(pred_cres_add_part[i], pow(2., 28), sk.key.get<Lvl1>());
        //     std::cout << "char_add: " << char_add << std::endl;

        //     uint32_t str_like = TFHEpp::tlweSymInt32Decrypt<Lvl1>(pred_cres_str[i], pow(2., 29), sk.key.get<Lvl1>());
        //     std::cout << "str_like: " << str_like << std::endl;

        //     std::cout << "pred_res_total[i]: " << pred_res_total[i] << std::endl;
        //     std::cout << "pred_cres_total_de[i]: " << pred_cres_total_de[i] << std::endl;

        //     std::cout << "pred_res1: " << pred_res1[i] << std::endl;
        //     uint32_t pred_cres1_de = TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres1[i], pow(2., 28), sk.key.get<Lvl1>());
        //     std::cout << "pred_cres1_de: " << pred_cres1_de << std::endl;

        //     std::cout << "pred_res2: " << pred_res2[i] << std::endl;
        //     uint32_t pred_cres2_de = TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres2[i], pow(2., 28), sk.key.get<Lvl1>());
        //     std::cout << "pred_cres2_de: " << pred_cres2_de << std::endl;

        //     std::cout << "pred_cres_add_total: "  << std::endl;
        //     uint32_t pred_cres_add_total_de = TFHEpp::tlweSymInt32Decrypt_print<Lvl1>(pred_cres_add_total[i], pow(2., 28), sk.key.get<Lvl1>());
        //     std::cout << "pred_cres_add_total_de: " << pred_cres_add_total_de << std::endl;


        //     std::cout << "pred_cres_part_de[i]: " << pred_cres_part_de[i] << std::endl;
    ...
    cout << "Per Like Time (ms): " << like_time / rows << "ms" << std::endl;
...
void aggregation(std::vector<TLWELvl1> &pred_cres, std::vector<uint32_t> &pred_res, size_t tfhe_n,
            std::vector<double> &extendedprice_data, std::vector<double> &discount_data_double,
             ...
             double &plian_aggregation_result , double &de_aggregation_result)
{
    ...
    plian_aggregation_result = plain_result;
...
void query_evaluation(size_t rows)
{
    ...
    uint32_t  ship_bits = 16, p_type_bits = 7, discount_bits = 6, ship_scale_bits, p_type_scale_bits, discount_scale_bits;
    ...
    predicate_evaluation(pred_cres_total, pred_res_total, pred_cres_part, pred_res_part, rows, p_type, ship_data, sk, filter_time);
    ...
    double plain_result = 100.0 * plain_aggregation_part_result / plain_aggregation_total_result ;
    double de_result = 100.0 * de_aggregation_part_result / de_aggregation_total_result ;
