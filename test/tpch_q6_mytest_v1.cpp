    ...
    std::vector<uint64_t> ship_data, TFHESecretKey &sk, double &filter_time)
{
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        my_greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres3[i], discount_bits, ek, ARITHMETIC, 2);
        ...
        my_greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher4, pred_cres4[i], discount_bits, ek, ARITHMETIC, 2);
