...
*/
bool relational_query14_he3db(size_t num, int threads, int agg_threads)
{
    if (!is_power_of_two(num))
        throw std::invalid_argument("rows must be a power of two for the rotate-sum aggregation");
    configure_threads(threads);
    std::cout << "Relational SQL Query14 HE3DB Baseline Test: " << std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
...
#pragma omp parallel for num_threads(threads) schedule(static)
    for (long i = 0; i < static_cast<long>(num); i++)
    {
        ...
        TLWELvl1 pre_res;
        greater_than<Lvl2>(shipdate_ciphers[i], predicate1_cipher, filter_res[i], ship_bits, ek, LOGIC);
        less_than<Lvl2>(shipdate_ciphers[i], predicate2_cipher, pre_res, ship_bits, ek, LOGIC);
        HomAND(filter_res[i], pre_res, filter_res[i], ek, LOGIC);
        greater_than<Lvl1>(ptype_ciphers[i], predicate3_cipher, pre_res, ptype_bits, ek, LOGIC);
        HomAND(filter_case_res[i], pre_res, filter_res[i], ek, LOGIC);
        less_than<Lvl1>(ptype_ciphers[i], predicate4_cipher, pre_res, ptype_bits, ek, LOGIC);
...
int main(int argc, char **argv)
{
    ...
    return relational_query14_he3db(rows, threads, agg_threads) ? 0 : 2;
}
