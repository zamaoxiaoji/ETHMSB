...
void predicate_evaluation(std::vector<TLWELvl1> &pred_cres, std::vector<uint32_t> &pred_res, size_t rows,
    ...
    size_t &predicate_errors, int threads)
{
...
#pragma omp parallel for num_threads(threads) schedule(static)
    for (long i = 0; i < static_cast<long>(rows); i++)
    {
...
#pragma omp parallel for num_threads(threads) schedule(static)
    for (long i = 0; i < static_cast<long>(rows); i++)
    {
        ethmsb_greater_than_equal<Lvl2>(ship_ciphers[i], pred_cipher1, pred_cres1[i], ship_bits, ek, LOGIC);
        ethmsb_less_than<Lvl2>(ship_ciphers[i], pred_cipher2, pred_cres2[i], ship_bits, ek, LOGIC);
        HomAND(pred_cres[i], pred_cres1[i], pred_cres2[i], ek, LOGIC);
        ethmsb_greater_than_equal<Lvl1>(discount_ciphers[i], pred_cipher3, pred_cres3[i], discount_bits, ek, LOGIC);
        HomAND(pred_cres[i], pred_cres[i], pred_cres3[i], ek, LOGIC);
        ethmsb_less_than_equal<Lvl1>(discount_ciphers[i], pred_cipher4, pred_cres4[i], discount_bits, ek, LOGIC);
        HomAND(pred_cres[i], pred_cres[i], pred_cres4[i], ek, LOGIC);
        ethmsb_less_than<Lvl1>(quanlity_ciphers[i], pred_cipher5, pred_cres5[i], quantity_bits, ek, LOGIC);
...
#pragma omp parallel for num_threads(threads) schedule(static) reduction(+:error_time)
    for (long i = 0; i < static_cast<long>(rows); i++) error_time += (pred_cres_de[i] == pred_res[i])? 0 : 1;
    ...
    predicate_errors = error_time;
    cout << "Predicate Evaluation Time: " << filter_time << " ms" << endl;
...
void aggregation(std::vector<TLWELvl1> &pred_cres, std::vector<uint32_t> &pred_res, size_t tfhe_n,
            std::vector<double> &extendedprice_data, std::vector<double> &discount_data_double,
             ...
             double &plain_result_out, double &encrypted_result_out,
             double &final_abs_error_out, double &repack_avg_error_out,
             int agg_threads)
{
    ...
    repack_avg_error_out = err / slots_count;
    printf("Repack average error = %f ~ 2^%.1f\n", repack_avg_error_out, std::log2(repack_avg_error_out));
...
#pragma omp parallel for schedule(static) reduction(+:plain_result)
    for (long i = 0; i < static_cast<long>(rows); i++)
    {
        plain_result += extendedprice_data[i] * discount_data_double[i] * pred_res[i];
    }
    cout << "Plain_result: " << plain_result << endl;
    plain_result_out = plain_result;
    encrypted_result_out = std::round(agg_result[0]);
    final_abs_error_out = std::abs(encrypted_result_out - plain_result_out);
    cout << "Encrypted query result: " << encrypted_result_out <<endl;
    cout << "Final absolute error: " << final_abs_error_out << endl;
    cout << "Aggregation Time: " << aggregation_time << " ms" << endl;

}

bool query_evaluation(size_t rows, int threads, int agg_threads)
{
    ...
    size_t predicate_errors = 0;
    double plain_result = 0;
    double encrypted_result = 0;
    double final_abs_error = 0;
    double repack_avg_error = 0;
    std::vector<TLWELvl1> pred_cres(rows);
    std::vector<uint32_t> pred_res(rows, 0);
    cout << "Records: " << rows << endl;
    cout << "Threads: " << threads << endl;
    cout << "Aggregation Threads: " << (agg_threads > 0 ? agg_threads : 1) << endl;
    predicate_evaluation(pred_cres, pred_res, rows, quanlity_data, discount_data, ship_data, sk, filter_time, predicate_errors, threads);
    aggregation(pred_cres, pred_res, Lvl1::n, extendedprice_data, discount_data_double, rows, sk, aggregation_time,
                plain_result, encrypted_result, final_abs_error, repack_avg_error,
                agg_threads);
    cout << "End-to-End Time: " << (filter_time + aggregation_time) / 1000 << " s" << endl;
    const bool correct = predicate_errors == 0 && final_abs_error <= 0.5 && repack_avg_error <= 0.25;
    cout << "BENCH_RESULT,Q6," << rows << "," << threads << ","
         << filter_time << "," << aggregation_time << "," << (filter_time + aggregation_time) << ","
         << predicate_errors << "," << repack_avg_error << ","
         << plain_result << "," << encrypted_result << "," << final_abs_error << ","
         << (correct ? 1 : 0) << endl;
    return correct;
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            cout << "Usage: " << argv[0] << " [--rows power_of_two] [--threads n] [--agg-threads n]" << endl;
            return 0;
        }
        else
        {
    ...
    return query_evaluation(rows, threads, agg_threads) ? 0 : 2;
}
